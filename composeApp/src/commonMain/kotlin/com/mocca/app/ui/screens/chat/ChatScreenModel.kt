    ...
    private fun syncDelegates() {
        screenModelScope.launch {
            combine(
                ...
                _state.value.copy(
                    messages = (messages as List<Message>).toImmutableList(),
                    childSessions = (childSessions as Map<String, Session>).toImmutableMap(),
                    childMessages = (childMessages as Map<String, List<Message>>).mapValues { it.value.toImmutableList() }.toImmutableMap(),
                    childStreamingText = (childStreamingText as Map<String, String>).toImmutableMap(),
                    isThinking = isThinking as Boolean,
                    thinkingContent = thinkingContent as String,
                    thinkingElapsedMs = thinkingElapsedMs as Long,
                    pendingPermission = pendingPermission as PermissionRequest?,
                    pendingQuestion = pendingQuestion as QuestionRequest?,
                    connectionStatus = connectionStatus as ConnectionStatus,
                    providerInfo = providerInfo as ProviderResponse?,
                    selectedProviderId = selectedProviderId as String,
                    selectedModelId = selectedModelId as String,
                    selectedVariantId = selectedVariantId as String?,
                    modes = modes as ImmutableList<Mode>,
                    selectedModeId = selectedModeId as String?,
                    isPlanMode = (selectedModeId as String?) == "plan",
                    modelName = modelName as String,
                    agentName = agentName as String,
                    maxTokens = maxTokens as Int,
                    recentModels = recentModels as ImmutableList<RecentModel>,
                    commands = commands as ImmutableList<Command>,
                    todos = (todos as List<Todo>).toImmutableList(),
                    showTimestamps = (userPreferences as UserPreferences).showTimestamps,
                    showTokenCounts = (userPreferences as UserPreferences).showTokenCounts,
                    // CRITICAL: Sync loading states from ChatStateStore
                    isLoading = isLoading as Boolean,
                    isSending = isSending as Boolean,
                    isSessionIdle = isSessionIdle as Boolean,
                    error = error as String?,
                    sessionDisposed = (sessionDisposed as String?) != null,
                    disposalReason = sessionDisposed as String?
                )
            }.collect { newState ->
